...
# Log Step
Add-LogMessage -message "Removing Provisioned Packages"
$allProvisionedApps = Get-AppxProvisionedPackage -Online
ForEach ($provApp in $allProvisionedApps) {
	if (-not ($allowedApps | Where-Object { $provApp.DisplayName -match $_ })) {
		Add-LogMessage -message "Removing app: $($provApp.DisplayName) aka $($provApp.PackageName)"
		Remove-AppxProvisionedPackage -Online -PackageName $provApp.PackageName
	}
}

# Log Step
Add-LogMessage -message "Removing All Users App Packages"
$allInstalledApps = Get-AppxPackage -AllUsers
